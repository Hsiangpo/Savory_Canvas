...
@router.post("/assets/image", response_model=Asset, status_code=status.HTTP_201_CREATED)
async def create_image_asset(
    ...
    file_name = f"{session_id}_{stem}{extension}"
    content = await file.read()
    file_path = storage.save_image(file_name, content)
    return service.create_image_asset(session_id=session_id, file_path=file_path, file_name=source_name)
